...
function harness({ optimized = true, legacy = false, permission = 'USUARIOS_GESTIONAR', assigned = true, emptyCatalogs = false } = {}) {
  ...
  const snapshotFactory = load(read('backend/src/services/ticket-detail-snapshot.service.js'), { readTable, sheetsRevisionTracker: tracker }, 'createTicketDetailSnapshot').createTicketDetailSnapshot;
  const group = load(read('backend/src/services/ticket-visit-group.service.js'), { ...errors, readTable, updateRow, nowIso: () => 'fixed-time', env: { sheetId: 'fixture' }, sheetsApi: { spreadsheets: { values: { get: async () => ({ data: { values: [['GrupoVisitaID','BoletaPrincipalUID','NumeroVisita','EsVisitaPrincipal','FirmaOrigen','FirmaFecha','EstadoEntregaFirma','UltimoErrorEntregaFirma','FirmaReenviadaEn']] } }) } } } }, 'ensureVisitGroupForTicket, groupSummary, ticketGroupId, ticketRootId, ticketVisitNumber');
  ...
  const ticketAccessHandlers = load(read('backend/src/modules/ticket-access.module.js'), { ...errors, readTable, pick }, 'ticketAccessHandlers').ticketAccessHandlers;
  const { assertTicketPayloadAccess } = load(read('backend/src/services/ticket-access.service.js'), { ...errors, readTable, pick }, 'assertTicketPayloadAccess');
  const ticketDeliveryHandlers = { get: async () => ({ candidates: true }) };
  load(read('backend/src/services/ticket-detail-read-optimization.patch.js'), { ...errors, pick, baseTicketHandlers, ticketMultiHandlers, ticketAccessHandlers, ticketDeliveryHandlers, assertTicketPayloadAccess, createTicketDetailSnapshot: optimized ? snapshotFactory : () => ({ read: readTable, locate: (rows, id) => rows.find(row => row.BoletaUID === id) }) }, '');
...
test('snapshot: evita lecturas duplicadas y descarta entradas tras cambios concurrentes conocidos', async () => {
  const h = harness();
  const snapshot = h.snapshotFactory();
  const [a, b] = await Promise.all([snapshot.read('Boletas'), snapshot.read('Boletas')]);
  assert.equal(a, b); assert.equal(h.reads.length, 1);
  h.tables.Boletas = h.tables.Boletas.map(row => ({ ...row, updated: true }));
  h.tracker.advance(new Set(['Boletas']));
  const c = await snapshot.read('Boletas');
  assert.notEqual(c, a); assert.equal(h.reads.length, 2);
  assert.equal(snapshot.locate(c, 'b1').updated, true);
});
