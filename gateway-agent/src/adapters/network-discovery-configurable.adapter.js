...
function envBoolean(value, fallback = false) {
  ...
  return /^(1|true|yes|y|si|sí|on)$/i.test(normalized);
}
...
export function parseOnvifDiscoveryXmlConfigurable(xml = '') {
...
async function discoverOnvif(timeoutMs) {
  ...
  return new Promise((resolve) => {
    ...
    socket.on('message', (message, remote) => {
      const parsed = parseOnvifDiscoveryXmlConfigurable(message.toString('utf8'));
      const ips = parsed.ips.length ? parsed.ips : [remote.address].filter(isValidIpv4);
...
async function probeWeb(ip, port, timeoutMs) {
  ...
  return new Promise((resolve) => {
    const request = transport.request({
      ...
      headers: {
        'User-Agent': 'DMS-Integration-Gateway/0.4',
...
function mergeCandidate(map, ip, allowedHosts, target, patch = {}) {
  if (!allowedHosts.has(ip)) return;
...
export function configurableCandidateToInventoryItem(candidate, macAddress = '', now = new Date().toISOString()) {
  ...
  return {
    ...
    metadata: {
      ...
      scanTarget: candidate.target?.spec || '',
      targetKind: candidate.target?.kind || '',
      publicTarget: candidate.target ? !candidate.target.privateOnly : false,
...
export class ConfigurableNetworkDiscoveryAdapter {
  constructor({ env = process.env } = {}) {
    this.name = 'NETWORK_DISCOVERY';
    this.maxHosts = boundedNumber(env.DMS_NETWORK_MAX_HOSTS, 1_024, 1, 2_000);
    this.allowPublicTargets = envBoolean(env.DMS_NETWORK_ALLOW_PUBLIC_TARGETS, false);
    const configured = String(env.DMS_NETWORK_TARGETS || env.DMS_NETWORK_CIDRS || '').trim();
    const plan = parseNetworkTargets(configured, {
      allowPublic: this.allowPublicTargets,
      maxHosts: this.maxHosts,
      maxPublicHostsPerTarget: 256,
    });
    this.targets = plan.targets;
    this.hosts = plan.hosts;
    this.allowedHosts = new Set(plan.hosts);
  ...
  capabilities() {
    return {
      ...
      configurableTargets: true,
      publicTargetsEnabled: this.allowPublicTargets,
      sourceSystems: ['NETWORK_DISCOVERY'],
    };
  }

  targetSpecs() {
    return this.targets.map((target) => target.spec);
  }
  ...
  async testConnection() {
    ...
    return {
      ...
      message: `${devices.length} posible(s) cámara(s) detectada(s) en ${this.hosts.length} IP configurada(s): ${this.targetSpecs().join(', ')}.`,
    };
  }
  ...
  async listDevices() {
    ...
    // WS-Discovery ayuda en la VLAN/subred local. Las respuestas solo se aceptan si están en el plan autorizado.
    (await discoverOnvif(this.onvifTimeoutMs)).forEach((response) => {
      const target = findNetworkTarget(this.targets, response.ip);
      mergeCandidate(candidates, response.ip, this.allowedHosts, target, {
    ...
    const tasks = [];
    this.hosts.forEach((ip) => this.scanPorts.forEach((port) => tasks.push({ ip, port })));
    const checks = await mapLimit(tasks, this.concurrency, async ({ ip, port }) => ({
      ip,
      port,
      open: await portOpen(ip, port, this.timeoutMs),
    }));
    checks.filter((item) => item.open).forEach(({ ip, port }) => {
      mergeCandidate(candidates, ip, this.allowedHosts, findNetworkTarget(this.targets, ip), {
        method: `TCP_${port}`,
        openPort: port,
      });
    });
    ...
    await mapLimit([...candidates.values()], Math.min(16, this.concurrency), async (candidate) => {
      ...
      if (webPort) {
        ...
        if (info) mergeCandidate(candidates, candidate.ip, this.allowedHosts, candidate.target, {
          method: `HTTP_${webPort}`,
          http: info,
        });
      }
    ...
    return [...candidates.values()]
      .map((candidate) => configurableCandidateToInventoryItem(candidate, macs.get(candidate.ip) || '', now))
      .filter(Boolean)
      .slice(0, 2_500);
  }
