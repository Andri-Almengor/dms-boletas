...
function KnowledgeCard({ record }) {
  ...
  >
    <div className="knowledge-card__top"><span className="knowledge-card__icon"><Icon name="menu_book" /></span><span className={`status-chip ${item.status === 'BORRADOR' ? 'status-chip--inactive' : 'status-chip--active'}`}>{item.status === 'BORRADOR' ? 'BORRADOR' : 'PUBLICADO'}</span></div>
    ...
    <div className="knowledge-card__resources"><span><Icon name="play_circle" /> {item.videos.length} video{item.videos.length === 1 ? '' : 's'}</span><span><Icon name="attach_file" /> {item.attachments.length} archivo{item.attachments.length === 1 ? '' : 's'}</span></div>
