...
function NavigationItem({ to, icon, label, end = false, onClick, prominent = false }) {
  return (
    <NavLink
      to={to}
      end={end}
    ...
    >
      <span className={prominent ? 'bottom-nav__fab' : ''}><Icon name={icon} filled={false} /></span>
      <span>{label}</span>
    </NavLink>
  );
}
  ...
  return (
    <div className="app-shell">
      <header className="top-bar">
        <button type="button" className="icon-button" onClick={() => setDrawerOpen(true)} aria-label="Abrir menú">
          <Icon name="menu" />
        </button>
        <NavLink to="/" className="top-bar__brand">DMS Boletas</NavLink>
        <NavLink to="/mas" className="avatar avatar--small" aria-label="Abrir perfil">
      ...
      <aside className={`side-drawer${drawerOpen ? ' is-open' : ''}`} aria-hidden={!drawerOpen}>
        <div className="side-drawer__profile">
          ...
          <div>
            ...
            <span>{canManageUsers ? 'Administrador' : 'Técnico'}</span>
          </div>
        </div>
      ...
      <nav className="bottom-nav" aria-label="Navegación principal">
        ...
        {canViewUsers && <NavigationItem to="/usuarios" icon="person_search" label="Usuarios" />}
        {canManageUsers && <NavigationItem to="/usuarios/nuevo" icon="add" label="Crear" prominent />}
        <NavigationItem to="/cambiar-contrasena" icon="lock_reset" label="Seguridad" />
