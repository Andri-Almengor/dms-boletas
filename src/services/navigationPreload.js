...
async function settleLimited(jobs = [], concurrency = IDLE_DATA_CONCURRENCY) {
...
function agendaPreload(sessionToken, isAdmin) {
  ...
  return once(`agenda|${range.from}|${range.to}|${sessionToken}|${isAdmin}`, async () => {
    ...
    return settleLimited(jobs);
  });
}

function knowledgePreload(sessionToken, canManageKnowledge) {
  return once(`knowledge|${sessionToken}|${canManageKnowledge}`, () => settleLimited([
...
function catalogJob(routes, payload, sessionToken) {
  return () => loadCatalogResource({ routes, payload, sessionToken });
}

function ticketFormPreload(sessionToken) {
  return once(`ticket-form|${sessionToken}`, () => settleLimited([
    catalogJob(MODULE_ROUTES.clients.list, { page: 1, pageSize: FORM_CLIENT_PAGE_SIZE, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.categories.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.failureTypes.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.deviceTypes.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.users.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.manufacturers.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.deviceManufacturers.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.models.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    () => requestAvailable(MODULE_ROUTES.config.get, {}, sessionToken),
  ]));
}

function maintenanceFormPreload(sessionToken) {
  return once(`maintenance-form|${sessionToken}`, () => settleLimited([
    catalogJob(MODULE_ROUTES.clients.list, { page: 1, pageSize: FORM_CLIENT_PAGE_SIZE, activo: true }, sessionToken),
    catalogJob(['users.assignment.list', 'users.list'], { page: 1, pageSize: 1000 }, sessionToken),
    catalogJob(MODULE_ROUTES.deviceTypes.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.manufacturers.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.models.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    catalogJob(MODULE_ROUTES.deviceManufacturers.list, { page: 1, pageSize: 1000, activo: true }, sessionToken),
    () => requestAvailable(['maintenance.config', 'mantenimientos.config'], {}, sessionToken),
  ]));
}
...
export function preloadNavigationData(pathname, {
  ...
  if (path === '/boletas/nueva') return ticketFormPreload(sessionToken);
  if (path === '/mantenimientos/nuevo') return maintenanceFormPreload(sessionToken);
 ...
 * abrirse. Se limita la concurrencia para no convertir la mejora de arranque
 * en una ráfaga de solicitudes contra Google Sheets.
 */
export async function preloadNavigationDataBatch(pathnames = [], context = {}) {
  ...
  return settleLimited(paths.map((path) => () => preloadNavigationData(path, context)));
}
