...
function EvidenceCard({ evidence, caseId, sessionToken }) {
  ...
  return <article className="case-evidence-card">
    ...
    <div><strong>{evidence.NombreArchivo || 'Evidencia'}</strong>{evidence.Nota && <span>{evidence.Nota}</span>}{error && <small className="is-error">{error}</small>}<a href={evidence.DriveURL} target="_blank" rel="noreferrer"><Icon name="open_in_new" />Abrir en Drive</a></div>
  </article>;
}
...
function dateDisplay(value) {
  ...
  return new Intl.DateTimeFormat('es-CR', { dateStyle: 'medium', timeStyle: value.includes('T') ? 'short' : undefined }).format(date);
}
...
export default function CustomerCaseDetailPage() {
  ...
  async function processCase(event) {
    ...
    try {
      ...
      setNotice(response.notificationSent
        ...
        : `El caso pasó a proceso y se creó la boleta #${updated.ticketNumber || updated.ticketId}. El correo quedó registrado con una advertencia: ${response.notificationWarning || 'revise la configuración SMTP'}`);
    } catch (saveError) {
  ...
  async function resend() {
    ...
    try {
      ...
      setNotice(response.sent ? 'El correo fue reenviado a los técnicos.' : `El reenvío no se completó: ${response.warning || 'revise SMTP'}`);
    } catch (sendError) {
  ...
  if (loading) return <div className="page"><div className="state-card state-card--loading"><Icon name="progress_activity" />Cargando caso...</div></div>;
  ...
  return <div className="page customer-case-detail-page">
    <header className="case-detail-heading">
      ...
      <div><span className="eyebrow">{item.number}</span><h1>{item.reason || 'Caso de cliente'}</h1><p>{item.client}</p></div>
    ...
    <div className="case-detail-layout">
      <div className="case-detail-main">
        <section className="case-detail-section">
          <header><span><Icon name="description" /></span><div><h2>Solicitud del cliente</h2><p>Información original enviada desde el formulario.</p></div></header>
        ...
        <section className="case-detail-section">
          <header><span><Icon name="photo_library" /></span><div><h2>Evidencias ({bundle.evidences?.length || 0})</h2><p>Imágenes aportadas por el cliente.</p></div></header>
          {bundle.evidences?.length ? <div className="case-evidence-detail-grid">{bundle.evidences.map((evidence) => <EvidenceCard key={evidence.CasoEvidenciaID} evidence={evidence} caseId={item.id} sessionToken={sessionToken} />)}</div> : <div className="case-section-empty"><Icon name="hide_image" /><span>El cliente no adjuntó evidencias.</span></div>}
        </section>
      </div>

      <aside className="case-detail-sidebar">
        {canProcess ? <form className="case-assignment-panel" onSubmit={processCase}>
          <header><span><Icon name="assignment_ind" /></span><div><h2>{item.state === 'EN_PROCESO' ? 'Actualizar asignación' : 'Preparar visita'}</h2><p>La boleta se crea al pasar el caso a proceso.</p></div></header>
          <fieldset><legend>Técnicos asignados *</legend><div className="case-technician-options">{technicians.map((user) => <label key={user.id} className={selectedTechnicians.includes(user.id) ? 'is-selected' : ''}><input type="checkbox" checked={selectedTechnicians.includes(user.id)} onChange={() => toggleTechnician(user.id)} /><span><strong>{user.name}</strong><small>{user.email || 'Sin correo'}</small></span><Icon name={selectedTechnicians.includes(user.id) ? 'check_circle' : 'radio_button_unchecked'} /></label>)}</div></fieldset>
          ...
          {selectedNames.length > 0 && <p className="case-selected-summary"><Icon name="engineering" />{selectedNames.join(', ')}</p>}
          <button className="button button--primary" disabled={saving}><Icon name={saving ? 'progress_activity' : item.state === 'EN_PROCESO' ? 'save' : 'play_circle'} />{saving ? 'Creando boleta y notificando...' : item.state === 'EN_PROCESO' ? 'Actualizar y notificar' : 'Pasar a en proceso'}</button>
        ...
        {item.ticketId && <section className="case-ticket-panel"><header><Icon name="confirmation_number" /><div><strong>Boleta #{item.ticketNumber || item.ticketId}</strong><span>Creada desde este caso</span></div></header><Link className="button button--secondary" to={`/boletas/${encodeURIComponent(item.ticketId)}`}><Icon name="open_in_new" />Abrir boleta</Link>{item.state === 'EN_PROCESO' && <button className="button button--ghost" type="button" onClick={resend} disabled={resending}><Icon name={resending ? 'progress_activity' : 'forward_to_inbox'} />{resending ? 'Reenviando...' : 'Reenviar correo a técnicos'}</button>}</section>}

        <section className="case-notification-panel"><h3>Notificaciones</h3><div><span>Al crear</span><strong className={String(item.EstadoNotificacionInicial).toUpperCase() === 'ENVIADO' ? 'is-success' : 'is-warning'}>{item.EstadoNotificacionInicial || 'PENDIENTE'}</strong></div><div><span>A técnicos</span><strong className={String(item.EstadoNotificacionTecnicos).toUpperCase() === 'ENVIADO' ? 'is-success' : 'is-warning'}>{item.EstadoNotificacionTecnicos || 'PENDIENTE'}</strong></div>{item.UltimoErrorNotificacion && <p>{item.UltimoErrorNotificacion}</p>}</section>
      </aside>
    </div>
  </div>;
}
