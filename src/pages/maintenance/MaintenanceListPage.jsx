...
function dateKey(value) { if (!value) return ''; const text = String(value); const isoMatch = text.match(/^\d{4}-\d{2}-\d{2}/); if (isoMatch) return isoMatch[0]; const date = new Date(value); if (Number.isNaN(date.getTime())) return ''; return `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`; }
function getId(row, fallback = '') { return String(pick(row, ['MantenimientoID','id','RowID'], fallback)); }
function maintenanceStatusFromQuery(value) { const normalized = String(value || '').trim().toUpperCase(); return ['FINALIZADO','FINALIZADA'].includes(normalized) ? 'FINALIZADO' : 'PENDIENTE'; }
...
function matchesFallbackFilters(row, status, query, filters) { const rowStatus = String(pick(row,['Estado'],'PENDIENTE')).toUpperCase(); if (rowStatus !== status) return false; const rowClient = String(pick(row,['Cliente','ClienteRef'],'')).trim(); if (filters.client && rowClient !== filters.client) return false; const rowDate = dateKey(pick(row,['Fecha'])); if (filters.dateFrom && (!rowDate || rowDate < filters.dateFrom)) return false; if (filters.dateTo && (!rowDate || rowDate > filters.dateTo)) return false; const normalizedQuery = query.trim().toLowerCase(); if (!normalizedQuery) return true; return [pick(row,['TituloMantenimiento']),rowClient,pick(row,['Responsables','Responsable']),pick(row,['DescripcionGeneral']),pick(row,['Ubicacion'])].join(' ').toLowerCase().includes(normalizedQuery); }
function maintenanceKey(row, index, source) { return getId(row, `${source}-${index}`); }
...
export default function MaintenanceListPage() {
  ...
  const resource = usePaginatedResource({
    ...
    normalizeResponse: (data) => normalizeItems(data).filter((row) => matchesFallbackFilters(row, status, appliedSearch, appliedFilters)),
    fetchPage: ({ page, pageSize, signal }) => requestAvailable(MODULE_ROUTES.maintenance.list, {
      page, pageSize, activo: true, status, estado: status, search: appliedSearch.trim(),
      cliente: appliedFilters.client, client: appliedFilters.client,
