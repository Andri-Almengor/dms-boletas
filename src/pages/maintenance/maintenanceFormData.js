...
export function createMaintenanceDevice(category = 'Cámaras') {
  return {
    ...
    id: '', ubicacionEquipoId: '', zona: '', categoria: category, nombre: '', modelo: '', serie: '',
    funcionamiento: '', enUso: '', estado: 'Correcto', observacion: '',
...
export function mapMaintenanceDevice(row) {
  ...
  return {
    ...
    zona: pick(row, ['Zona', 'UbicacionEspecifica']), categoria: pick(row, ['Categoria'], 'Cámaras'),
    nombre: pick(row, ['NombreDispositivo']), modelo: pick(row, ['Modelo']), serie: pick(row, ['Serie']),
...
export function maintenanceDevicePayload(device, maintenanceId) {
  return {
    ...
    Zona: device.zona, Categoria: device.categoria, NombreDispositivo: device.nombre,
    Modelo: device.modelo, Serie: device.serie, Funcionamiento: device.funcionamiento,
