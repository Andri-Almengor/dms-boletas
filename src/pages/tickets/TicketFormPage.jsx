...
function options(rows, valueKeys, labelKeys) {
  return rows.map((row) => toOption(row, valueKeys, labelKeys)).filter(Boolean);
}

function find(rows, value, keys) {
...
function mapForm(data) {
  ...
  return {
    ...
    fecha: String(pick(row, ['Fecha'], EMPTY.fecha)).slice(0, 10),
...
function payload(form, boletaUid, estado = 'PENDIENTE') {
  return {
    ...
    // El backend y la plantilla siguen usando Descripcion / <<[Descripción]>>.
...
export default function TicketFormPage({ mode = 'create' }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (a.status === 'fulfilled') setLocations(normalizeItems(a.value));
      if (b.status === 'fulfilled') setContacts(normalizeItems(b.value));
    });
  ...
  useEffect(() => {
    ...
    requestAvailable(MODULE_ROUTES.clients.equipmentLocationsList, { ubicacionId: form.ubicacionId, activo: true, pageSize: 500 }, sessionToken)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      requestAvailable(MODULE_ROUTES.tickets.autosave, payload(form, boletaUid), sessionToken)
  ...
  const opt = {
    clients: options(catalogs.clients, ids.clients, ['Nombre', 'Clientes', 'RazonSocial']),
    categories: options(catalogs.categories, ids.categories, ['Nombre']),
    failures: options(catalogs.failures, ids.failures, ['Nombre']),
    devices: options(catalogs.devices, ids.devices, ['Nombre']),
    locations: options(locations, ['UbicacionID', 'id'], ['Nombre']),
    equipment: options(equipmentLocations, ['UbicacionEquipoID', 'id'], ['Nombre']),
    supervisors: contacts
      .filter((item) => toBoolean(pick(item, ['EsSupervisor'], true), true))
      .map((item) => {
        const option = toOption(item, ['ContactoID', 'id'], ['Nombre']);
        return option ? { ...option, label: `${option.label}${pick(item, ['Correo']) ? ` · ${pick(item, ['Correo'])}` : ''}` } : null;
      })
      .filter(Boolean),
  };

  const relationIds = catalogs.relations
    .filter((item) => String(pick(item, ['TipoDispositivoID'])) === form.tipoDispositivoId && toBoolean(pick(item, ['Activo'], true), true))
    .map((item) => String(pick(item, ['FabricanteID'])));
  opt.manufacturers = options(
    relationIds.length
      ? catalogs.manufacturers.filter((item) => relationIds.includes(String(pick(item, ['FabricanteID']))))
  ...
  opt.models = options(
    catalogs.models.filter((item) => (
      (!form.tipoDispositivoId || String(pick(item, ['TipoDispositivoID'])) === form.tipoDispositivoId)
      && (!form.fabricanteId || String(pick(item, ['FabricanteID'])) === form.fabricanteId)
    )),
    ids.models,
    ['Nombre'],
  );

  const technicians = catalogs.users
    .map((item) => {
  ...
  function choose(event, rows, idKeys, idField, nameField, nameKeys, reset = {}, extra) {
    ...
    const row = find(rows, value, idKeys);
    setForm((current) => ({
      ...current,
      [idField]: value,
  ...
  function validate(index) {
    ...
    if (index === 2 && (!form.tipoDispositivoId || !form.nombreDispositivo.trim())) {
  ...
  async function uploadAssets(uid) {
    if (form.firma?.startsWith('data:image/')) {
      await requestAvailable(['boletas.signature.upload'], {
        boletaUid: uid,
  ...
  async function saveBase() {
    const result = await requestAvailable(
      ...
      payload(form, boletaUid),
      sessionToken,
    );
    const uid = pick(recordData(result), ['BoletaUID', 'boletaUid', 'TicketUID', 'id'], boletaUid);
  ...
  async function action(type) {
    const message = STEPS.map((_, index) => validate(index)).find(Boolean);
    if (message) return setError(message);
    ...
    try {
      ...
      if (type === 'test') {
        await requestAvailable(MODULE_ROUTES.tickets.testFinalize, { boletaUid: uid, testMode: true }, sessionToken);
      }
      if (type === 'pdf') {
  ...
  function openModal(type) {
    setModal({
      type,
      values: {
        nombre: '', descripcion: '', correo: '', puesto: '', telefono: '', direccion: '', notas: '', imagenReferenciaURL: '',
      },
    });
    setModalError('');
  }
  ...
  return (
    <div className="page page--narrow ticket-form-page">
      ...
      <section className="ticket-progress">
        <div><strong>Paso {step + 1} de 8</strong><span>{progress}% completado</span></div>
      ...
      <section className="form-card ticket-form-card">
        <div className="form-card__heading">
          ...
          <div><h2>Paso {step + 1}: {STEPS[step][0]}</h2><p>{STEPS[step][1]}</p></div>
        </div>
        ...
        <div className="stack-form">
          {step === 0 && <>
            ...
            <div className="ticket-form-grid">
              <DependentSelect label="Categoría" name="categoriaId" value={form.categoriaId} options={opt.categories} required canAdd={manageCatalogs} onAdd={() => openModal('category')} onChange={(e) => choose(e, catalogs.categories, ids.categories, 'categoriaId', 'categoria', ['Nombre'])} />
              <DependentSelect label="Tipo de falla" name="tipoFallaId" value={form.tipoFallaId} options={opt.failures} required canAdd={manageCatalogs} onAdd={() => openModal('failure')} onChange={(e) => choose(e, catalogs.failures, ids.failures, 'tipoFallaId', 'tipoFalla', ['Nombre'])} />
            </div>
            <div className="ticket-form-grid ticket-form-grid--three">
              <Field label="Fecha" type="date" name="fecha" value={form.fecha} onChange={update} />
            ...
            <Field label="Horas totales" type="number" step="0.01" name="horasTotales" value={form.horasTotales} onChange={update} hint="Cálculo automático, incluso cruzando medianoche." />
          </>}

          {step === 1 && <>
            <DependentSelect label="Cliente" name="clienteId" value={form.clienteId} options={opt.clients} required onChange={(e) => choose(e, catalogs.clients, ids.clients, 'clienteId', 'cliente', ['Nombre', 'Clientes'], { ubicacionId: '', ubicacion: '', ubicacionEquipoId: '', ubicacionEquipo: '', supervisorId: '', supervisor: '', correoSupervisor: '' }, (row) => ({ correoCliente: pick(row, ['CorreoGeneral', 'Correo']) }))} />
            <div className="ticket-form-grid">
              <DependentSelect label="Ubicación" name="ubicacionId" value={form.ubicacionId} options={opt.locations} disabled={!form.clienteId} canAdd={createOperational && Boolean(form.clienteId)} onAdd={() => openModal('location')} onChange={(e) => choose(e, locations, ['UbicacionID', 'id'], 'ubicacionId', 'ubicacion', ['Nombre'], { ubicacionEquipoId: '', ubicacionEquipo: '' })} />
            ...
            <DependentSelect label="Supervisor" name="supervisorId" value={form.supervisorId} options={opt.supervisors} disabled={!form.clienteId} canAdd={createOperational && Boolean(form.clienteId)} onAdd={() => openModal('supervisor')} onChange={(e) => choose(e, contacts, ['ContactoID', 'id'], 'supervisorId', 'supervisor', ['Nombre'], {}, (row) => ({ correoSupervisor: pick(row, ['Correo']) }))} />
            <div className="ticket-form-grid">
              <Field label="Correo supervisor" type="email" name="correoSupervisor" value={form.correoSupervisor} onChange={update} readOnly={!hasPermission('CLIENTES_EDITAR')} />
              <Field label="Correo cliente" type="email" name="correoCliente" value={form.correoCliente} onChange={update} readOnly={!hasPermission('CLIENTES_EDITAR')} />
            </div>
          </>}

          {step === 2 && <>
            <DependentSelect label="Tipo de dispositivo" name="tipoDispositivoId" value={form.tipoDispositivoId} options={opt.devices} required canAdd={manageCatalogs} onAdd={() => openModal('device')} onChange={(e) => choose(e, catalogs.devices, ids.devices, 'tipoDispositivoId', 'tipoDispositivo', ['Nombre'], { fabricanteId: '', fabricante: '', modeloId: '', modelo: '' })} />
            <Field label="Nombre del dispositivo" name="nombreDispositivo" value={form.nombreDispositivo} onChange={update} required />
            <div className="ticket-form-grid">
              <DependentSelect label="Fabricante" name="fabricanteId" value={form.fabricanteId} options={opt.manufacturers} disabled={!form.tipoDispositivoId} canAdd={manageCatalogs && Boolean(form.tipoDispositivoId)} onAdd={() => openModal('manufacturer')} onChange={(e) => choose(e, catalogs.manufacturers, ids.manufacturers, 'fabricanteId', 'fabricante', ['Nombre'], { modeloId: '', modelo: '' })} />
              <DependentSelect label="Modelo" name="modeloId" value={form.modeloId} options={opt.models} disabled={!form.tipoDispositivoId || !form.fabricanteId} canAdd={manageCatalogs && Boolean(form.fabricanteId)} onAdd={() => openModal('model')} onChange={(e) => choose(e, catalogs.models, ids.models, 'modeloId', 'modelo', ['Nombre'])} />
            </div>
          ...
          {step === 7 && <>
            ...
            <Field label="Correos CC" name="correosCC" value={form.correosCC} onChange={update} hint="Separe varios correos con coma." />
