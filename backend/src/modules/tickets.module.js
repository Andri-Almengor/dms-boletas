...
function sameIds(left, right) {
  const a = normalizedIds(left); const b = normalizedIds(right);
...
async function enrichTicket(row) {
  ...
  const assigned = tables.BoletaAsignados
    ...
    .map((item) => {
      ...
      return { ...item, NombreCompleto: name, Nombre: name, NombreUsuario: String(user?.NombreUsuario || '').trim(), Correo: String(user?.Correo || '').trim() };
    });
  const evidences = tables.EvidenciasBoleta.filter((item) => String(item.BoletaUID) === String(row.BoletaUID) && item.Activo !== false);
...
export const ticketHandlers = {
  ...
  mediaGet: async ({ payload }) => { const media = await resolveTicketMedia(payload); return { mediaId: media.recordId, kind: media.kind, fileId: media.fileId, ...await downloadAsDataUrl(media.fileId, media.mimeType) }; },
