...
import {
  buildMaintenanceTicketGroups,
  fitMaintenanceTicketCell,
  MAINTENANCE_TICKET_SPLIT_LIMITS,
...
export function stagedGeneratedTicketUid(maintenanceId, groupKey) {
...
async function syncTicketEvidences(ctx, bundle, ticketId, maintenanceId, group) {
  ...
  selectedImages.forEach((image, index) => {
    ...
    const device = group.devices.find((item) => String(item.EvidenciaMantenimientoID) === String(image.DispositivoMantenimientoRef));
    const row = {
      BoletaUID: ticketId,
...
function ticketPayload(bundle, group, draft, unchangedFinalized, existing) {
  ...
  return {
    payload: {
      ...
      TipoDispositivo: fitMaintenanceTicketCell(
        [...new Set(group.devices.map((device) => clean(device.Categoria || device.TipoDispositivo)).filter(Boolean))].join(', '),
        1000,
      ),
...
export async function prepareStagedTicketPlan(maintenanceId) {
  const id = clean(maintenanceId);
  if (!id) throw badRequest('No se indicó el mantenimiento para preparar las boletas.');
  await ensureColumns();
  ...
  return {
    ...
    groups: groups.map((group, index) => ({
      key: group.key,
      order: index + 1,
      ticketId: stagedGeneratedTicketUid(id, group.key),
      date: group.date,
      partIndex: group.partIndex || 1,
      partCount: group.partCount || 1,
      technicianIds: asArray(group.technicianIds),
      deviceIds: group.devices.map((device) => clean(device.EvidenciaMantenimientoID)),
...
export async function processStagedTicketItem(ctx, maintenanceId, groupKey) {
  const id = clean(maintenanceId);
  const key = clean(groupKey);
  if (!id || !key) throw badRequest('No se indicó la parte de la boleta que se debe procesar.');
  await ensureColumns();
  const bundle = await loadBundle(id);
  const group = buildMaintenanceTicketGroups(bundle).find((candidate) => clean(candidate.key) === key);
  if (!group) throw badRequest('La parte de la boleta ya no existe en el plan actual del mantenimiento.');

  const ticketId = stagedGeneratedTicketUid(id, group.key);
  ...
  await syncTicketEvidences(ctx, bundle, ticketId, id, group);
  ...
  return {
    ...
    pdfUrl: clean(delivery?.pdfUrl || stored.PDF_Url || stored.PDFUrl),
    date: group.date,
    partIndex: group.partIndex || 1,
    partCount: group.partCount || 1,
    deviceCount: group.devices.length,
