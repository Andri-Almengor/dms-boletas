...
for(const [key,names] of Object.entries(maintenanceAliases)) {
  ...
  else if(['ticketGenerationTest','signatureReset','signatureTestLink'].includes(key)) permission=['USUARIOS_GESTIONAR','MANTENIMIENTOS_GESTIONAR','MANTENIMIENTOS_ELIMINAR'];
  const handler = key === 'signatureLink'
    ? maintenanceSignatureHandlers.link
    : key === 'signatureReset'
      ? maintenanceSignatureHandlers.reset
