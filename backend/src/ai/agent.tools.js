...
export const TOOL_DECLARATIONS=Object.freeze({
  ...
  get_maintenance:fn('get_maintenance','Obtiene resumen completo de un mantenimiento: estado, fechas, categorías, zonas, supervisores, avance planificado/registrado y resumen de evidencias.',{maintenanceId:{type:'string'}},['maintenanceId']),
  get_maintenance_devices:fn('get_maintenance_devices','Lista dispositivos de un mantenimiento con zona, equipo, estado, operatividad, fechas de creación/actualización y cantidades de evidencias antes/después.',{maintenanceId:{type:'string'},query:{type:'string'},type:{type:'string'},observationsOnly:{type:'boolean'},limit:{type:'integer',minimum:1,maximum:50},offset:{type:'integer',minimum:0}},['maintenanceId']),
  ...
  search_maintenance_evidence:fn('search_maintenance_evidence','Busca evidencias de mantenimiento con total real, paginación, fechas, dispositivo/zona, tipo MIME y clasificación ANTES/DESPUÉS. Devuelve imágenes/videos como attachments protegidos.',{
    maintenanceId:{type:'string'},deviceIds:{type:'array',items:{type:'string'}},type:{type:'string'},query:{type:'string'},
    stage:{type:'string',enum:['ANTES','DESPUES']},mimeCategory:{type:'string',enum:['IMAGE','VIDEO','PDF','OTHER']},
    limit:{type:'integer',minimum:1,maximum:50},offset:{type:'integer',minimum:0},...COMMON_DATE_PROPERTIES,
  },['maintenanceId']),
