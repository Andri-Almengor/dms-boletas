...
function evidenceCategoryClause(category,alias='e'){
  ...
  if(value==='OTHER')return `(
    ${mime} NOT LIKE 'image/%' AND ${mime} NOT LIKE 'video/%' AND ${mime}<>'application/pdf'
...
export async function searchTicketEvidence(ctx,args={}){
  ...
  if(category==='SIGNATURE'){
    const reference=clean(args.ticketId||args.ticketNumber,250);
    if(!reference) throw badRequest('Para consultar una firma indique ticketId o ticketNumber.');
    const ticket=await accessibleTicket(ctx,reference);
    const signature=clean(ticket.__signatureFile)?protectedAttachment(ctx,{
      fileId:ticket.__signatureFile,mimeType:ticket.__signatureMime||'image/png',
      scopeId:ticket.uid,evidenceId:ticket.uid,kind:'signature',
      title:'Firma de la boleta #'+(ticket.number||ticket.uid),subtitle:'Firma asociada a la boleta',
      entityType:'ticket',entityId:ticket.uid,
    }):null;
    return {
      modelData:{
        total:signature?1:0,totalShown:signature?1:0,mimeCategory:'SIGNATURE',
        ticket:{uid:ticket.uid,number:ticket.number||ticket.uid,title:ticket.title||''},
        items:signature?[{id:'signature:'+ticket.uid,name:'Firma',mimeType:ticket.__signatureMime||'image/png',category:'SIGNATURE'}]:[],
      },
  ...
  if(ticketRef){params.push(ticketRef);clauses.push(`(b."BoletaUID"=${params.length} OR b."BoletaID"=${params.length})`);}
  if(clean(args.uploaderId)){params.push(clean(args.uploaderId,250));clauses.push(`e."CreadoPor"=${params.length}`);}
  if(clean(args.uploaderName)){params.push(like(args.uploaderName));const p='
  const ticket = await accessibleTicket(ctx, args.ticketId || args.number);
  const rows = await many(
...
+params.length;clauses.push(`(uploader."NombreCompleto" ILIKE ${p} ESCAPE '\\' OR uploader."NombreUsuario" ILIKE ${p} ESCAPE '\\')`);}
  if(clean(args.query)){
    params.push(like(args.query));const p='
  const ticket = await accessibleTicket(ctx, args.ticketId || args.number);
  const rows = await many(
    `SELECT a."Accion" AS action, a."UsuarioID" AS "userId",
            a."UsuarioNombre" AS "userName", a."Fecha" AS date
...
+params.length;
    clauses.push(`(
      e."Nombre" ILIKE ${p} ESCAPE '\\' OR e."Nota" ILIKE ${p} ESCAPE '\\'
      OR e."NombreArchivo" ILIKE ${p} ESCAPE '\\' OR b."Titulo" ILIKE ${p} ESCAPE '\\'
      OR b."Cliente" ILIKE ${p} ESCAPE '\\'
    )`);
  }
  ...
  const counted=await one(
    `SELECT COUNT(*)::bigint AS total
       ...
       LEFT JOIN "Usuarios" uploader ON uploader."__valid"=TRUE AND uploader."UsuarioID"=e."CreadoPor"
      WHERE ${where}`,
    params,'ai.ticketEvidence.search.count',
  );
  const queryParams=[...params,pageLimit(args.limit,30),pageOffset(args.offset)];
  const rows=await many(
    `SELECT e."EvidenciaID" AS id,e."BoletaUID" AS "ticketId",b."BoletaID" AS "ticketNumber",
            b."Titulo" AS "ticketTitle",b."Cliente" AS client,e."Nombre" AS name,e."Nota" AS note,
            e."NombreArchivo" AS "fileName",e."MimeType" AS "mimeType",e."TipoMedio" AS "mediaType",
            e."TamanoBytes" AS size,e."FechaCreacion" AS "createdAt",e."CreadoPor" AS "createdBy",
            COALESCE(NULLIF(uploader."NombreCompleto",''),uploader."NombreUsuario",e."CreadoPor") AS "uploadedBy",
      ...
      ORDER BY e."FechaCreacion" DESC NULLS LAST,e."__db_id" DESC
      LIMIT ${queryParams.length-1} OFFSET ${queryParams.length}`,
    queryParams,'ai.ticketEvidence.search.items',
  );
  const items=rows.map(row=>({
    id:row.id,ticketId:row.ticketId,ticketNumber:row.ticketNumber||row.ticketId,ticketTitle:row.ticketTitle||'',
    client:row.client||'',name:row.name||row.fileName||'Evidencia',note:clean(row.note,1600),
    fileName:row.fileName||'',mimeType:row.mimeType||'application/octet-stream',mediaType:row.mediaType||'',
    size:row.size||'',createdAt:row.createdAt||'',createdBy:row.createdBy||'',uploadedBy:row.uploadedBy||row.createdBy||'',
    deviceId:row.deviceId||'',
  }));
  const attachments=rows.map(row=>protectedAttachment(ctx,{
    fileId:row.__file,mimeType:row.mimeType,scopeId:row.ticketId,evidenceId:row.id,kind:'evidence',
    title:row.name||row.fileName||'Evidencia',
    subtitle:['Boleta #'+(row.ticketNumber||row.ticketId),row.client,row.uploadedBy,row.createdAt].filter(Boolean).join(' · '),
    entityType:'ticket',entityId:row.ticketId,
  })).filter(Boolean);
  return {
    modelData:{total:Number(counted?.total||0),totalShown:items.length,mimeCategory:category||'',period,items},
    attachments,
    entities:items.slice(0,30).map(item=>entity('ticket',item.ticketId,'Boleta #'+item.ticketNumber,'/boletas/'+encodeURIComponent(item.ticketId))),
    sources:items.slice(0,10).map(item=>source('ticket',item.ticketId,'Boleta #'+item.ticketNumber+' · '+item.name,'/boletas/'+encodeURIComponent(item.ticketId))),
