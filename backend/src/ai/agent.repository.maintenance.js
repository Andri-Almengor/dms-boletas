...
export async function resolveMaintenanceReference(ctx,args={}){
  ...
  const candidates=rows.map(row=>({id:row.id,title:row.title||'Mantenimiento',clientId:row.clientId||'',client:row.client||'',location:row.location||'',status:row.status||'',date:row.date||'',finishedAt:row.finishedAt||''}));
  if(!chosen)return{
  ...
  return{
    modelData:{resolved:true,maintenance:candidates.find(row=>row.id===chosen.id),candidates},
...
export async function getMaintenanceEvidence(ctx,args={}){
  ...
  if(clean(args.type)){params.push(like(args.type));const p='
  const rows=await many(
  ...
  const items=rows.map(row=>({
    id:row.id,name:row.name||'Evidencia',note:clean(row.note,1600),stage:row.stage||'',mimeType:row.mimeType||'application/octet-stream',
  ...
  return {
    ...
    context:{
      lastMaintenanceId:maintenance.id,lastMaintenanceName:maintenance.title||'',lastClientId:maintenance.clientId||'',lastClientName:maintenance.client||'',
      ...(items.length&&items.every(item=>item.stage===items[0].stage)&&items[0].stage?{lastEvidenceStage:items[0].stage.toUpperCase().replace('DESPUES','DESPUES')}:{})
    },
  };
}
...
+params.length;
    clauses.push(`(d."NombreDispositivo" ILIKE ${p} ESCAPE '\\' OR d."TipoDispositivo" ILIKE ${p} ESCAPE '\\' OR d."Categoria" ILIKE ${p} ESCAPE '\\' OR d."Zona" ILIKE ${p} ESCAPE '\\')`);
  }
  const rows=await many(
    `SELECT d."EvidenciaMantenimientoID" AS id,d."NombreDispositivo" AS name,
  ...
  if(!rows.length)return{modelData:{resolved:false,ambiguous:false,maintenance:{id:maintenance.id,title:maintenance.title},candidates:[],message:'No se encontró el dispositivo dentro del mantenimiento.'}};
  ...
  return{
    modelData:{resolved:true,maintenance:{id:maintenance.id,title:maintenance.title},device:candidates.find(row=>row.id===chosen.id),candidates},
...
+params.length;clauses.push(`(d."TipoDispositivo" ILIKE ${p} ESCAPE '\\' OR d."Categoria" ILIKE ${p} ESCAPE '\\')`);}
  if(clean(args.stage)){
    const stage=normalizeText(args.stage).includes('desp')?'Despues':normalizeText(args.stage).includes('antes')?'Antes':'';
