...
export async function searchKnowledgeDocumentChunks(ctx, args = {}) {
  ...
  if (documentId) {
    scopedDocument = await visibleDocument(ctx, documentId);
    queueKnowledgeDocumentIndexing(scopedDocument, ctx.user?.UsuarioID || '');
    const scopedState=extractionState(scopedDocument.extractionStatus);
    if(scopedState!=='READY'){
      console.info('[ai-knowledge] '+JSON.stringify({
        event:scopedState==='EXTRACTION_FAILED'?'ai_knowledge_error':'ai_knowledge_not_indexed',
        documentId:scopedDocument.id,
        articleId:scopedDocument.articleId,
        extractionStatus:scopedDocument.extractionStatus||'UPLOADED',
      ...
      return {
        modelData:{
          ...
          document:{
            ...
            articleId:scopedDocument.articleId,
            name:scopedDocument.name||'Documento',
            articleTitle:scopedDocument.articleTitle||'Artículo',
            mimeType:scopedDocument.mimeType||'',
            extractionStatus:scopedDocument.extractionStatus||'UPLOADED',
            canRetryIndexing:scopedState==='EXTRACTION_FAILED'&&canRetryIndexing(ctx,scopedDocument),
          },
          items:[],
        },
        sources:[knowledgeDocumentSource(ctx,scopedDocument)],
        context:documentContext(scopedDocument),
      };
    }
  } else if (articleId) {
